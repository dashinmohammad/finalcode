...
Serial.print("Bluetooth connected");
  SerialAT.begin(9600,SERIAL_8N1,18,5,false);
  delay(3000);
...
void loop() {
 ...
 SerialBT.println("The button state is ");
  SerialBT.println(buttonstate);
   serialData = SerialBT.read();
      if(serialData == '1' || serialData == '2' || buttonstate == HIGH)
     {
         ...
         DBG("GPRS status:", res ? "connected" : "not connected");
          ...
          if(serialData == '1')  {
          res = modem.sendSMS(SMS_TARGET, String("Auto-Detection -- Heart Rate Alert has been detected at  ") + gsmLoc); }
          else if(serialData == '2')  {
          res = modem.sendSMS(SMS_TARGET, String("Auto-Detection -- Fall Alert has been detected at  ") + gsmLoc); }
          else  {
          res = modem.sendSMS(SMS_TARGET, String("Manual-Detection -- SOS Button has been pressed at ") + gsmLoc); }
          DBG("SMS:", res ? "OK" : "fail");
        #endif
...
/*
  ...
  //SerialBT.println("Publishing data to Ubidots Cloud");
